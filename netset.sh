#!/bin/bash
# If found in root of USB stick:
# config.yaml - process for drawBridge network config
# *.pco - process certificates generated by CertMaker7.ps
# pco.patch - process files inside to patch IoTGateway application
# updater.patch - process files inside to patch usb updater application
# demokit.config - process file to update an AAAPERIO router WAN interface

pathusb=/media/usb # do not change, managed by Linux
demoIP="10.11.12.1" # this is the LAN IP of the AAAPERIO router for updates to the WAN interface
...
cd $pathusb
for i in ./*.next; do mv -i "$i" "${i%.next}"; done
cd $whereami

echo $(date) "$hostn: UNMOUNTING & EXECUTING SHUTDOWN" >>$pathusb/log.txt
pumount usb
sleep 2
shutdown now
